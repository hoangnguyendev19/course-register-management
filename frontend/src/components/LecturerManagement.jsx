...
const LecturerManagement = () => {
  ...
  const handleDelete = async (id) => {
    const data = await LecturerAPI.deleteLecturer(id);
    if (data) {
  ...
  const handleAdd = async (user) => {
    const data = await LecturerAPI.addLecturer(user);
    if (data) {
  ...
  const handleEdit = async (id, newUser) => {
    const data = await LecturerAPI.updateLecturer(id, newUser);
    if (data) {
  ...
  const lecturerColumn = [
    { field: "id", headerName: "Mã giảng viên", width: 120 },
    { field: "fullName", headerName: "Tên giảng viên", width: 300 },
