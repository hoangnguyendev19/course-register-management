...
const EditClassModal = ({
  ...
  useEffect(() => {
    if (clazz) {
      setName(clazz.name);
      setCourse(clazz.course);
      setLecturer(clazz.lecturer);
      setCapacity(clazz.capacity);

      if (clazz.type === "Lý thuyết") {
        setType("theory");
        setDisableTheory(true);
      } else {
        setType("practice");
        setDisableTheory(false);
        setTheory(clazz.theory);
      }

      setStartDate(dayjs(clazz.startDate.split("/").reverse().join("-")));
      setEndDate(dayjs(clazz.endDate.split("/").reverse().join("-")));
    }
  }, [clazz]);
  ...
  const handleSubmit = () => {
    const newClass = {
      ...
      capacity,
      type,
      theory: type === "theory" ? "" : theory,
      startDate: startDate.format("YYYY-MM-DD"),
      endDate: endDate.format("YYYY-MM-DD"),
    };
    handleEdit(clazz.id, newClass);
    setOpen(false);
  };
